...
char *strSHA2(char *filename, char *filepath)
{
    char real_sha_path[1000];

    strcpy(real_sha_path, sha_path);
    strcat(real_sha_path, filepath);
    strcat(real_sha_path, "/");
    strcat(real_sha_path, filename); //path is modified to go into further depth directories
    
#if	defined(O_BINARY)
    int	fd = open(real_sha_path, O_RDONLY | O_BINARY, 0);
#else
    int	fd = open(real_sha_path, O_RDONLY, 0);
#endif
